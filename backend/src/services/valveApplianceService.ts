...
export async function getValveApplianceStatusList(params: GetValveApplianceParams): Promise<{
  ...
  const cacheKey = `valves_appliances:${JSON.stringify(filterParams)}`;
  ...
  return memoryCache.getOrFetch(
    ...
    async () => {
      ...
      // 2. Fetch Users with Farms, Boundaries, NodeRedDeviceDetails, and Device capability flags in parallel
      ...
      if (allFarmIds.length > 0) {
        const envReadings = await prisma.$queryRaw<any[]>`
          SELECT r.*
          FROM (SELECT unnest(ARRAY[${Prisma.join(allFarmIds)}]::text[]) as fid) f
          CROSS JOIN LATERAL (
            SELECT 
        ...
        `.catch((err) => {
          console.error('Error fetching batch environmental readings in valveApplianceService LATERAL:', err);
          return [];
        });
    ...
    15, // 15 seconds fresh TTL
    params.forceFresh,
    45 // 45 seconds stale grace
  );
}
